...
def main():
    ...
    if next_phase_num != current_phase_num:
        next_phase_header = re.search(rf"^## Phase {next_phase_num}\s+[—-]\s+(?P<title>.+)$", updated_master, re.MULTILINE)
        require(next_phase_header is not None, f"Unable to locate next phase header for Phase {next_phase_num}.")
        updated_master, _ = CURRENT_PHASE_RE.subn(
            f"- Current phase: **Phase {next_phase_num} — {next_phase_header.group('title').strip()}**",
            updated_master,
            count=1,
        )

        current_phase_header = re.search(rf"^## Phase {current_phase_num}\s+[—-]\s+(?P<title>.+)$", updated_master, re.MULTILINE)
        require(current_phase_header is not None, f"Unable to locate completed phase header for Phase {current_phase_num}.")
        updated_master, _ = COMPLETED_PHASE_RE.subn(
            f"- Completed phase: **Phase {current_phase_num} — {current_phase_header.group('title').strip()}**",
            updated_master,
            count=1,
        )
