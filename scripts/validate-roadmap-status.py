...
def extract_all_master_steps(text: str):
    steps = []
    for line in text.splitlines():
        match = MASTER_TASK_RE.match(line.strip())
        if match:
            steps.append(match.group('step').upper())
    return steps
...
def main():
    ...
    all_steps = extract_all_master_steps(master_text)
    require(current_step in all_steps, f"Current task step '{current_step}' is missing from docs/MASTER_STATUS.md.")
    current_index = all_steps.index(current_step)
    expected_next = all_steps[current_index + 1] if current_index + 1 < len(all_steps) else current_step
