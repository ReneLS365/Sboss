...
public sealed class RoadmapStatusGuardrailTests
{
    ...
    public void ValidationScript_PassesForCurrentRepoState()
    {
        var result = RunValidation(
            ...
            taskId: "P1D-ECONOMY-TRANSACTION-SERVICE");
    ...
    public void ValidationScript_FailsWhenMultipleTasksAreInProgress()
    {
        ...
        var result = RunValidation(WriteTempFile(plans), ResolveRepoPath("docs/MASTER_STATUS.md"), taskId: "P1D-ECONOMY-TRANSACTION-SERVICE");
    ...
    public void ValidationScript_FailsWhenNextTaskSkipsAhead()
    {
        ...
        masterStatus = masterStatus.Replace("- Next task: **1E — Contract job state machine**", "- Next task: **1F — Company/job application services**", StringComparison.Ordinal);

        var result = RunValidation(ResolveRepoPath("PLANS.md"), WriteTempFile(masterStatus), taskId: "P1D-ECONOMY-TRANSACTION-SERVICE");
    ...
    {
        var result = RunValidation(ResolveRepoPath("PLANS.md"), ResolveRepoPath("docs/MASTER_STATUS.md"), taskId: "P1D-ECONOMY-TRANSACTION-SERVICE");
