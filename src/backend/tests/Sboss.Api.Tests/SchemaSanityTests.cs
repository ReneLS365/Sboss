...
public sealed class SchemaSanityTests
{
    ...
    [Fact]
    public void ContractJobsMigrationContainsRequiredTables()
    {
        var migrationPath = ResolveMigrationPath(ContractJobsMigrationFileName);
        var migration = File.ReadAllText(migrationPath);

        Assert.Contains("CREATE TABLE IF NOT EXISTS contract_jobs", migration, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("CREATE TABLE IF NOT EXISTS contract_job_transitions", migration, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("contract_job_transitions_idempotency_unique", migration, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("resulting_version", migration, StringComparison.OrdinalIgnoreCase);
    }
    ...
    [Fact]
    public void CrewPayoutSnapshotsMigrationContainsRequiredTables()
    {
        var migrationPath = ResolveMigrationPath(CrewPayoutSnapshotsMigrationFileName);
        var migration = File.ReadAllText(migrationPath);

        Assert.Contains("CREATE TABLE IF NOT EXISTS crew_payout_settlements", migration, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("CREATE TABLE IF NOT EXISTS crew_payout_settlement_members", migration, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("crew_payout_settlement_members_settlement_fk", migration, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void WearAndTearMigrationContainsIntegrityColumn()
    {
        var migrationPath = ResolveMigrationPath(WearAndTearMigrationFileName);
        var migration = File.ReadAllText(migrationPath);

        Assert.Contains("total_integrity_bps", migration, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("inventory_items_total_integrity_bps_non_negative", migration, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void LoadoutAndFogMigrationContainsRequiredTables()
    {
        var migrationPath = ResolveMigrationPath(LoadoutAndFogMigrationFileName);
        var migration = File.ReadAllText(migrationPath);

        Assert.Contains("CREATE TABLE IF NOT EXISTS loadout_sessions", migration, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("CREATE TABLE IF NOT EXISTS loadout_session_items", migration, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("CREATE TABLE IF NOT EXISTS fog_of_war_states", migration, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("loadout_sessions_account_seed_unique", migration, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("fog_of_war_states_account_seed_key_unique", migration, StringComparison.OrdinalIgnoreCase);
    }
    ...
    public void PostgresTestFixture_AppliesFullMigrationChainBeforeSeed()
    {
        ...
        Assert.Contains("NpgsqlDataSourceRegistry.DisposeTrackedDataSourcesAsync();", fixture, StringComparison.Ordinal);
        ...
        var trackedPoolIndex = fixture.IndexOf("NpgsqlDataSourceRegistry.DisposeTrackedDataSourcesAsync();", StringComparison.Ordinal);
        ...
        Assert.True(trackedPoolIndex >= 0 && clearAllPoolsIndex > trackedPoolIndex, "Fixture must dispose tracked NpgsqlDataSource instances before clearing global connection pools.");
